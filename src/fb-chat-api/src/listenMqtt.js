	...
	utils
		...
		.then(function (resData) {
			...
			if (oldFBMQTTMatch) {
				...
				logged("login", `Got this account's message region: ${region}`);
			} else {
				...
				if (newFBMQTTMatch) {
					...
					if (legacyFBMQTTMatch) {
						...
						region = new URL(mqttEndpoint).searchParams.get("region").toUpperCase();
						logged("login", `Cannot get sequence ID with new RegExp. Fallback to old RegExp (without seqID)...`);
						logged("login", `Got this account's message region: ${region}`);
						logged("login", `[Unused] Polling endpoint: ${legacyFBMQTTMatch[6]}`);
					} else {
						logged("login", "Cannot get MQTT region & sequence ID.");
						noMqttData = html;
					}
				}
			}
